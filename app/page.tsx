...
const addAnalysisStep = (step: string, status: 'processing' | 'completed' = 'processing') => {
  ...
  setCurrentAnalysis(prev => {
    ...
    if (existingStepIndex >= 0) {
      updatedSteps[existingStepIndex] = {
        ...updatedSteps[existingStepIndex],
        status,
        duration: status === 'completed' ? timestamp - updatedSteps[existingStepIndex].timestamp : undefined
      }
    } else {
      updatedSteps.push({ step, timestamp, status })
    }
...
const startAnalysis = (type: 'text' | 'voice', model?: string) => {
  ...
  addAnalysisStep('Initializing request')
}

const completeAnalysis = (reasoning?: string, tokenCount?: number) => {
  if (!currentAnalysis) return
  ...
  setAnalysisData(prev => [completedAnalysis, ...prev.slice(0, 9)]) // Keep last 10
  ...
  try {
    ...
    dc.onmessage = (event) => {
      ...
      if (message.type === "response.audio.delta") {
        setIsSpeaking(true);
        if (currentAnalysis) {
          addAnalysisStep('Receiving audio response')
        }
      } else if (message.type === "response.done") {
        setIsSpeaking(false);
        if (currentAnalysis) {
          addAnalysisStep('Voice response completed', 'completed')
          completeAnalysis('Voice interaction completed', message.usage?.total_tokens || 0)
        }
        ...
        if (currentAnalysis) {
          addAnalysisStep('AI response generated')
        }
      }
...
const handleSuggestionClick = (suggestion: string) => {
  if (isVoiceMode && isSessionActive) {
    ...
    startAnalysis('voice', 'gpt-4o-realtime')
    addAnalysisStep('Sending voice suggestion to Realtime API')
    ...
    addAnalysisStep('Triggering AI response')
    // Trigger response
    sendClientEvent({
      type: "response.create"
    });

    addAnalysisStep('Waiting for voice response', 'completed')
    return;
  }

  // Handle text mode suggestions with analysis
  if (model === "coach") {
    startAnalysis('text', 'coach')
    addAnalysisStep('Processing coach request')
    ...
    addAnalysisStep('Coach response queued', 'completed')
    completeAnalysis('Coach request processed', 15)
  } else {
    startAnalysis('text', model)
    addAnalysisStep('Preparing text request')
    
    const provider = models.find((m) => m.id === model)?.provider || "openai"
    const effectiveWebSearch = provider === "openai" ? useWebSearch : false

    addAnalysisStep('Sending to AI model')
    sendMessage({
      text: suggestion,
      metadata: {
    ...
    addAnalysisStep('Streaming response', 'completed')
  }
}
